...
void PMDWIN::otodasiz(QQ *qq)
{
	...
	cx2 = cx + ((signed long long)cx) / 256 * ax;
...
int PMDWIN::ppc_load2(char *filename)
{
	...
	f_read(&fp, pcmbuf, size, &byteRead);
	f_close(&fp);
...
int WINAPI PMDWIN::music_load(char *filename)
{
	...
	f_read(&fp, musbuf, sizeof(musbuf), (unsigned int *)&size);
...
int WINAPI PMDWIN::fgetmemo(char *dest, char *filename, int al)
{
	...
	if(filename != NULL) {
		if(*filename != '\0') {
			...
			f_read(&fp, mmlbuf, mdata_def*1024, (unsigned int *)&size);
...
{
	signed long long	_pos;
	int		us;

	_pos = (signed long long)pos * 1000;			// (ms -> usec への変換)
