...
int PPZ8::Load(char *filename, int bufnum)
{
	...
	if((f_open(&fp, filename, FA_READ)) != FR_OK) {
		...
		if(NOW_PCM_CATE) {	// PZI 読み込み
			f_read(&fp, &pziheader, sizeof(PZIHEADER), &byteRead);
			...
			f_read(&fp, &pviheader, sizeof(PVIHEADER), &byteRead);
...
bool PPZ8::SetOntei(int ch, uint ontei)
{
	...
	channelwork[ch].PCM_ADD_H = (int)(
			(unsigned long long)((channelwork[ch].PCM_ADDS_H << 16) + channelwork[ch].PCM_ADDS_L)
